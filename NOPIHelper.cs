...
{
    private static POILogger wl = POILogFactory.GetLogger("DEBUG");


    #region 从datatable中将数据导出到excel
    /// <summary>
    /// DataTable导出到Excel的MemoryStream，适用小数据量导出
    /// </summary>
    /// <param name="dtSource">源DataTable</param>
    /// <param name="strHeaderText">表头文本</param>
    static MemoryStream ExportStream(DataTable dtSource, string strHeaderText)
    {
    ...
    /// <summary>
    /// DataTable导出到Excel文件流，适用大数据量导出
    /// </summary>
    /// <param name="dtSource">源DataTable</param>
    /// <param name="strHeaderText">表头文本</param>
    static void ExportStream(DataTable dtSource, string strHeaderText, FileStream fs)
    {
    ...
    public static void ExportDTtoExcel(DataTable dtSource, string strHeaderText, string strFileName)
    {
        ...
        {
            using (MemoryStream ms = ExportStream(dtSource, strHeaderText))
            {
        ...
        else
        {
            ...
            using (FileStream fs = new FileStream(strFileName, FileMode.Create, FileAccess.Write))
            {
                ExportStream(dtSource, strHeaderText, fs);
            }
        }
    }
    #endregion

    public static MemoryStream ExportDTtoStream(DataTable dtSource, string strHeaderText)
    {
        if (dtSource.Columns.Count < 256 && dtSource.Rows.Count < 65536)
        {
            return ExportStream(dtSource, strHeaderText);
        }
        else
        {
            //大文件的导出，此段代码未覆盖测试   ********
            string temp = System.Environment.GetEnvironmentVariable("TEMP", EnvironmentVariableTarget.User);
            string tempFile = DateTime.Now.ToLongTimeString() + "xlsx";

            using (FileStream fs = File.OpenRead(temp + tempFile))
            {
                ExportStream(dtSource, strHeaderText, fs);
                int length = (int)fs.Length;
                byte[] data = new byte[length];
                fs.Position = 0; fs.Read(data, 0, length);
